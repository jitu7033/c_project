...
void editorSetStatusMessage(const char *fmt, ...){ //... is a variadic function it mean you pass many argument at a time 
  va_list ap; 
  va_start(ap,fmt);

  // to helpus to make our own printf() style function
...
void editorMoveCursor(int key)
{
  ...
  case ARROW_LEFT:
    ...
    { // when you are in left of the line then click left you go up end
      E.cy--;
  ...
  case ARROW_RIGHT:
    ...
    { // when you are right end and click right key you move to the next line 0th col
      E.cy++;
      E.cx = 0;
    }
    break;
...
void editorProcessKeypress() {
  ...
  static int quit_time = KILO_QUIT_TIMES;
  int c = editorReadKey();
  switch (c) {
    ...
    case CTRL_KEY('q'):
      ...
      if(E.dirty && quit_time > 0){
        editorSetStatusMessage("! warning !! file has been unsaved changes." 
                                "press Ctrl-Q %d more times to quit " , quit_time);
        quit_time--;
        return;
      }
      write(STDOUT_FILENO, "\x1b[2J", 4);
...
void initEditor()
{
  ...
  E.rowoff = 0;  // we initilize zero it mean we start the top of the file by default
  ...
  E.statusmsg[0] = '\0'; // so no msg will display by default 
  E.statusmsg_time = 0;
  E.dirty = 0;
